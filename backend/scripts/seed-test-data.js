...
async function seedDatabase() {
  try {
    ...
    const testEmployees = [
      { email: 'group1_emp1@test.com', first_name: 'John', last_name: 'Smith', role: 'employee' },
      { email: 'group1_emp2@test.com', first_name: 'Alice', last_name: 'Johnson', role: 'employee' },
      { email: 'group1_emp3@test.com', first_name: 'Bob', last_name: 'Williams', role: 'employee' },
      { email: 'group2_emp1@test.com', first_name: 'Carol', last_name: 'Davis', role: 'employee' },
      { email: 'group2_emp2@test.com', first_name: 'David', last_name: 'Miller', role: 'employee' },
      { email: 'group2_emp3@test.com', first_name: 'Emma', last_name: 'Wilson', role: 'employee' },
    ];
      ...
      if (checkResult.rows.length === 0) {
        const insertResult = await pool.query(
          `INSERT INTO users (email, password_hash, first_name, last_name, role, department, email_verified, is_active)
           ...
           RETURNING id`,
          [emp.email, password, emp.first_name, emp.last_name, emp.role, emp.first_name.includes('group1') ? 'Group 1' : 'Group 2']
        );
    ...
    console.log(`✓ Created ${45} test schedule entries (3 employees × 15 days × 2 groups)`);
